...
#define LCD_PinsOut TRISB&=~(BIT6|BIT7|BIT8|BIT9|BIT12); //TRISA&=~(BIT0|BIT1|BIT3|BIT4|BIT5);  
#define LCDInitState LATB|=(BIT9|BIT12); // CE=RST="1"


// Mapping des signaux SPI1 (LCD Nokia)
//#define MapLCDSCLK _RP6R=8 // SCK2 mappée sur RP6=RB6=LCDSCLK   patte 15
...
// positionnement de la patte Data/Commande
  ...
  for (i=0; i<8; i++)
  {
   ...
   Data <<= 1;
   Lcd_CLK=1; // pour assurer duée mini 125ns de état haut avec FCY = 40MHz
   Lcd_CLK=1;
   Lcd_CLK=1;
   Lcd_CLK=1;
   Lcd_CLK=0;
